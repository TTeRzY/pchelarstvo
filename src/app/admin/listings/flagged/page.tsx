...
export default function AdminFlaggedListingsPage() {
  ...
  const loadListings = async () => {
    setLoading(true);
    try {
      const data = await adminClient.getFlaggedListings();
  ...
  if (listings.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-2">{t('listings.allClear')}</h3>
        <p className="text-gray-600">{t('listings.noFlagged')}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-4">
        <p className="text-red-700 font-medium">
          🚩 {listings.length} {t('listings.requireAttention')}
        </p>
      </div>
